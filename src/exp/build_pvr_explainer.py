...
def heatmap_meaning():
    return (
        '<div class="scene"><div class="head"><h3>열지도가 검증한 것과 그 의의 — 쉬운 설명</h3></div>'
        '<div class="grid3">'
        '<div class="step" style="border-left:4px solid var(--gt)"><div class="n">무엇을 확인하려 했나</div>'
        '<p>반사실 실험(②)은 모델이 언덕·표적·모서리에 반응한다는 것을 보였다. 그 반응이 입력 화면의 <b>바로 그 자리</b>에서 오는지를 열지도로 확인하려 했다. 실행 전에 통과 기준을 정해 두었다: 관련 구조가 미끼보다 밝아야 한다(H1).</p></div>'
        '<div class="step" style="border-left:4px solid var(--model)"><div class="n">무엇이 나왔나</div>'
        '<p>세 씬 모두 기준을 넘지 못했다. 그래디언트 지도는 하늘·뒷벽·잡동사니가 가장 밝고, 점수와 무관한 숫자로 그린 지도와 거의 같았다(상관 0.64–0.85). 어텐션 지도는 예측된 공이 자기 과거를 압도적으로 읽고, 관련 구조는 균일보다 어두웠다. 다만 관련 구조가 미끼보다는 조금 밝다는 방향성(12셀 중 11)은 남았고, 벽에서는 그것도 깨졌다.</p></div>'
        '<div class="step" style="border-left:4px solid var(--rel)"><div class="n">그래서 무엇을 뜻하나</div>'
        '<p><b>모델이 구조를 안 쓴다는 뜻이 아니다.</b> 반사실 실험이 쓴다는 것을 이미 보였다. 이 열지도 방식이 그 의존을 화면 위 한 자리로 찍어 내지 못했다는 뜻이다. 공이 밝게 보이는 것은 지도가 엉터리가 아니라는 점검일 뿐, 우리가 물은 구조(언덕·표적·모서리)에 대한 답은 아니다. 사전에 정한 대로 열지도는 "후보를 고르는 신호"로 내리고, 반사실 편집을 주지표로 확정했다.</p></div>'
        '</div>'
        '<div class="prose" style="margin-top:1rem"><p><b>왜 이것이 값진 결과인가.</b> 첫째, 벤치마크 설계에 대한 답이다. 열지도를 주지표로 삼았다면 "모델은 언덕을 보지 않는다"는 틀린 결론을 냈을 것이다. 같은 모델·같은 조건에서 반사실은 "언덕 높이를 쓴다"(높이를 바꾸면 예측이 바뀜)고 말했고, 열지도는 "언덕 자리가 두드러지지 않는다"(배경보다 어둡고 미끼보다 밝지 않음)고 말했다. 둘이 충돌하면 조작에 기반한 반사실이 옳다. 주의할 점: 어텐션 지도에서 <b>공</b>이 밝게 보이는 것은 맞고 기대된 일이다 — 다음 프레임의 공을 그리려면 지금 공의 위치가 가장 강한 단서이므로. 우리가 물은 것은 공이 아니라 <b>공의 미래를 결정할 구조</b>(언덕·표적·모서리)를 읽는가였고, 그 구조에 대한 참고는 화면을 고르게 읽었을 때의 0.2–0.5배로 작았다. "덜 참고"가 곧 "안 씀"은 아니므로(작은 참고로도 결과를 바꿀 수 있다), 정확한 결론은 "모델이 구조를 안 본다"가 아니라 "이 지도로는 구조 의존을 국소화할 수 없다"이다. 둘째, 열지도가 왜 실패하는지의 원인을 좁혔다. 지도가 점수와 무관한 지도와 닮았다는 것은 미분값의 대부분이 "이 입력이 공의 미래를 정한다"가 아니라 "네트워크가 이 입력에 민감하다"에서 온다는 뜻이며, 이는 확산 모델의 단일 단계 민감도가 국소화 도구로 부적합하다는 구체적 근거다. 셋째, 어텐션에서 남은 방향성은 새 가설을 낳았다: 모델은 인과적으로 관련된 물체가 아니라 "진행 방향에 있는 공 같은 물체"를 읽는 것일 수 있다(벽 0.13 &lt; 미끼 공 0.63). 이는 미끼의 물체 종류를 통제하는 후속 실험으로 검증할 수 있다.</p>'
        '<p><b>다음에 열지도를 쓴다면.</b> 지도를 그리는 대신 지도가 가리켜야 할 자리를 직접 바꿔 넣는다. 조건 잠재에서 언덕 영역의 토큰만 다른 조건의 것으로 교체하고 같은 seed로 끝까지 생성해, 실제 편집 효과의 몇 %가 재현되는지를 잰다. 이렇게 하면 열지도가 반사실과 같은 언어(효과 크기, 신뢰구간)로 읽히고, 정합 검사가 따로 필요 없다.</p></div></div>')
