...
def heatmap_meaning():
    return (
        ...
        '<p><b>모델이 구조를 안 쓴다는 뜻이 아니다.</b> 반사실 실험이 쓴다는 것을 이미 보였다. 이 열지도 방식이 그 의존을 화면 위 한 자리로 찍어 내지 못했다는 뜻이다. 사전에 정한 대로 열지도는 "후보를 고르는 신호"로 내리고, 반사실 편집을 주지표로 확정했다.</p></div>'
        '</div>'
        '<div class="prose" style="margin-top:1rem"><p><b>왜 이것이 값진 결과인가.</b> 첫째, 벤치마크 설계에 대한 답이다. 열지도를 주지표로 삼았다면 "모델은 언덕을 보지 않는다"는 틀린 결론을 냈을 것이다. 같은 모델·같은 조건에서 반사실은 "본다", 열지도는 "안 본다"고 말했고, 조작에 기반한 반사실이 옳다. 둘째, 열지도가 왜 실패하는지의 원인을 좁혔다. 지도가 점수와 무관한 지도와 닮았다는 것은 미분값의 대부분이 "이 입력이 공의 미래를 정한다"가 아니라 "네트워크가 이 입력에 민감하다"에서 온다는 뜻이며, 이는 확산 모델의 단일 단계 민감도가 국소화 도구로 부적합하다는 구체적 근거다. 셋째, 어텐션에서 남은 방향성은 새 가설을 낳았다: 모델은 인과적으로 관련된 물체가 아니라 "진행 방향에 있는 공 같은 물체"를 읽는 것일 수 있다(벽 0.13 &lt; 미끼 공 0.63). 이는 미끼의 물체 종류를 통제하는 후속 실험으로 검증할 수 있다.</p>'
